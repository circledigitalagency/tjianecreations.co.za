...
import { sendCustomOrderStatusEmail } from "~/email.server";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
	...
	if (!["complete", "completed"].includes(yocoData.status)) {
		return redirect(`/custom-orders/${params.id}/balance`);
	}

	// Update to complete, record balance payment
	await pool.query(
		`UPDATE custom_orders
     SET status = 'payment_complete', balance_due = 0
	...
	try {
		...
		console.error("Balance confirmation email failed:", e);
	}

	return json({ order });
}

export default function BalanceSuccess() {
		...
		<MainLayout>
			<div className="px-16 py-32 max-w-lg mx-auto text-center">
				...
				<p className="text-bark-mid font-light leading-relaxed mb-8">
					Thank you, {order.customer_name}! Your {order.bag_style} will be
					dispatched shortly. You'll receive a shipping notification once it's
					on its way.
				</p>
				<Link
					to="/"
